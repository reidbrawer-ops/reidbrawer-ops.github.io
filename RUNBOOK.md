...
- `<head>` boilerplate (fonts, favicons, manifest, theme-color, OG/Twitter) is
  **hand-maintained per page, but verified**. It used to be generated by
  `scripts/build.mjs`, which was retired in July 2026: it was a second generator
  that also owned the `<header>`, and it had drifted badly — it still carried the
  pre-dropdown flat nav and a "More" menu missing the legal links, so running it
  would have silently reverted the live header. The header now has exactly one
  owner (`sync-header.js`), and the footer stays deliberately per-page (below).
  In place of build.mjs's `--check`, `check-redesign-invariants.mjs` compares the
  head chrome on every page against **index.html as the reference**, plus
  `compare.html` and the paddle detail *template* (which stands in for its 514
  outputs). Anchoring on a reference instead of hardcoded values is the point:
  bump the favicon `?v=` or the font URL everywhere and it still passes; bump it
  on *some* pages and it names the ones you missed. Adding a new page? It is
  picked up automatically — nothing to register.
  - Two deliberate exemptions, both asserted rather than assumed: `404.html`
    ships no canonical/OG because it is noindexed (and the check verifies that
    noindex is still there, since it is what makes the exemption safe), and
    `twitter:image` is not required because `twitter:card` falls back to
    `og:image` per Twitter's spec — `compare.html` relies on that.
...
| `node scripts/check-redesign-invariants.mjs` | The HTML/JS contract `validate` can't see: the map ids that are dereferenced unguarded, the `data-pq-*` revenue attributes, the verbatim Amazon Associates sentence, the single-definition rule for `vendorLinkFor`, the sync-script anchors, the fonts/tokens, and the shared `<head>` boilerplate on every page (build.mjs's old `--check`). Add an assertion here whenever you find a hook whose loss would be silent. |
